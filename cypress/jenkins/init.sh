...
install_prerequisites() {
  ...
  if ! command -v tofu &>/dev/null; then
    ...
    local tofu_version="${TOFU_VERSION:-1.11.5}"
...
clone_qa_infra() {
  if [[ -d "${QA_INFRA_DIR}/.git" ]]; then
    ...
    cd "${QA_INFRA_DIR}" && git fetch origin && git checkout -qf "${QA_INFRA_BRANCH}" && git reset --hard "origin/${QA_INFRA_BRANCH}" || true
  else
...
if [[ "${1:-}" == "destroy" ]]; then
  ...
  cypress_exit=0
  docker run --rm -t \
    --name "${RANCHER_HOST:-dashboard-e2e}" \
    --shm-size=2g \
    --env-file "${HOME}/.env" \
    -e NODE_PATH="" \
    -v "${HOME}:/e2e" \
  ...
  # Copy results to workspace (jrm merge happens inside the container)
  dashboard_dir="${HOME}/dashboard"
  cp "${dashboard_dir}/results.xml" "${WORKSPACE:-.}/" 2>/dev/null || true
  mkdir -p "${WORKSPACE:-.}/html"
  cp -r "${dashboard_dir}/cypress/reports/html/"* "${WORKSPACE:-.}/html/" 2>/dev/null || true
