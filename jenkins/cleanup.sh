...
rancherRunning=$( docker inspect -f '{{.Config.Running}}' ${RANCHER_CONTAINER_NAME} 2>/dev/null )
rancherImageExist=$( docker inspect -f '{{.Config.Image}}' ${RANCHER_CONTAINER_NAME} 2>/dev/null )
cypressRunning=$( docker inspect -f '{{.Config.Running}}' ${CYPRESS_CONTAINER_NAME} 2>/dev/null )
